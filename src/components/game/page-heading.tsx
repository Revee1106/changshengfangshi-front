...
export function PageHeading({ eyebrow, title, description, badge }: PageHeadingProps) {
  return (
    <header className="mb-6 flex flex-col justify-between gap-3 md:flex-row md:items-end">
      <div>
        {eyebrow ? <p className="text-xs font-medium text-jade-700">{eyebrow}</p> : null}
        <h1
          className={
            eyebrow
              ? "mt-1 text-2xl font-semibold text-ink-900 md:text-3xl"
              : "text-2xl font-semibold text-ink-900 md:text-3xl"
          }
        >
          {title}
        </h1>
